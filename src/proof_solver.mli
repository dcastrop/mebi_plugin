...
module Make : (Log : Logger.SLogger) (E : Encoding.SEncoding) -> sig
  module W : sig
    module M : sig
      ...
      module Enc : sig
        type t = Rocq_monad.Make(Log)(Rocq_context.Default)(E).Enc.t
      ...
      module Errors : sig
        type t = Rocq_monad_utils.Make(Log)(Rocq_context.Default)(E).Errors.t =
          | LTS_Empty
          | LTS_Incomplete of string
          | Not_Bisimilar
          | Invalid_Ind_Kind of Rocq_ind.kind
  ...
  module State_ : sig
    module type S = sig
      type t = { term : M.Enc.t; pp : string option }
    ...
    module State : sig
      type t = Model_state.Make(Log)(M.Enc).State.t = {
        term : M.Enc.t;
        pp : string option;
      }
    ...
    module States : (State : S) -> sig
      module Set : sig
        type elt = State.t
        type t = Model_state.Make(Log)(M.Enc).States(State).Set.t
  ...
  module Label_ : sig
    ...
    module Label : sig
      type t = Model_label.Make(Log)(M.Enc).Label.t = {
        term : M.Enc.t;
        pp : string option;
        is_silent : bool option;
      }
    ...
    module Labels : (Label : S) -> sig
      module Set : sig
        type elt = Label.t
        type t = Model_label.Make(Log)(M.Enc).Labels(Label).Set.t
  ...
  module Labels : sig
    ...
    module Weak : sig
      type t = Wrapper.Make(Log)(Rocq_context.Default)(E).Weak.t =
        | Option of M.Enc.t
    ...
    module Config : sig
      ...
      type weak_args =
            Wrapper.Make(Log)(Rocq_context.Default)(E).Config.weak_args =
        { a : Weak.t option
        ; b : Weak.t option
        }
    ...
    module Graph : (T0 : sig
      ...
      module A : sig
        ...
        val size : t' -> int
        val update : t' -> Model.Actions.elt -> D.t -> unit
        val to_string : t' -> string
      end
      ...
      module Make : (_ : Y_Args) -> sig
        val next_to_visit : t -> V.elt
        val update_to_visit : t -> V.elt -> unit
        val update_states : t -> V.t -> t
        val get_new_constrs : V.elt -> M.Constructor.t list M.mm
        val get_new_states : t -> V.elt -> V.t M.mm
        val build : t -> t M.mm
      end

      module Extract : (_ : Z_Args) -> sig
        val pp : V.elt -> string option
        val state : V.elt -> Model.States.elt
        val states : unit -> Model.States.t
        val terminals : unit -> Model.States.t
        val label : A.key -> Model.Labels.elt
    ...
    module Command : sig
      ...
      type t = Wrapper.Make(Log)(Rocq_context.Default)(E).Command.t =
      ...
      and rocq_pair =
            Wrapper.Make(Log)(Rocq_context.Default)(E).Command.rocq_pair =
        { a : rocq_args
        ; b : rocq_args
        }
