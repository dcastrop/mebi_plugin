...
module RCP = struct
  ...
  module KS90 = struct
    ...
    (** [reachable_blocks ?params actions pi] is the subset of partition [pi] containing blocks that contain the destination states of the [actions].
        ...
        @param pi is the partition of blocks from which we extract and return a subset from.
        @return a partition that contains all of the blocks within [pi] that contain a destination state of [actions]. *)
    ...
    (** [inner_loop ?params b1 b2 b pi changed] is the main inner loop of the [KS90] algorithm. It handles the result of [split], and determines if the split produced a refinement of [pi].
        ...
        @param pi is the partition containing all states to be grouped by the algorithm with those they are bisimilar with..
        @param changed is a referenced used to signify if [pi] has changed, and if the algorithm should terminate. *)
    let inner_loop
          ?(params : Params.log = default_params)
          (b1 : Block.t)
          (b2 : Block.t)
    ...
    (** [main_loop_body ?params (alphabet,edges) pi changed] is the main outer loop of the [KS90] algorithm. It handles iterating through each block in [pi], and trying to split the block by finding an action not shared by all (via [split]), the result of which is handled by [inner_loop].
        @param ?params contains parameters for logging and formatting.
        @param (alphabet,edges) are the (merged) alphabet and edges of the (merged) system being checked.
        @param pi is the partition contianing all states.
        @param changed is used to denote whether a refinement has occured at all in the current iteration.
        @return unit (nothing), since [pi] (and [changed]) are references that are updated. *)
    let main_loop_body
          ?(params : Params.log = default_params)
    ...
    (** [run ?params input] handles running the alogrithm for either term Minimization or checking Bisimilarity.
        @param ?params contains parameters for logging and formatting.
        @param input is [of_bisim_input], which facilitates Minimization (via [Minimize]) or Bisimilarity (via [ToMerge] or [Merged]).
        @return [of_bisim_result], either for [Minimize] or [Bisimilarity], which must be futher processed via [result].*)
    let run ?(params : Params.log = default_params) (input : of_bisim_input)
      : of_bisim_result
      =
    ...
    (** [split_bisimilar ?params (s_states,t_states) map_of_states pi] is ...
        @param ?params contains parameters for logging and formatting.
        @param (s_states,t_states) are the states of the (pre-merged) fsms being checked.
        @param map_of_states maps the original (pre-merged) states to the post-merged states.
        @param pi is the partition containing all blocks of states.
        @return a pair of partitions containing the bisimilar and non-bisimilar states. *)
    let split_bisimilar
          ?(params : Params.log = default_params)
          ((s_states, t_states) : States.t * States.t)
          (map_of_states : (state, state) Hashtbl.t)
