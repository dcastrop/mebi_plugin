...
module MakeEConstrMap (Ctx : Context.TYPE) :
...
module Term = struct
  ...
  module MakeFromEnc (Enc : Encoding.TYPE) : TYPE with type t = Enc.t = struct
    type t = Enc.t
    (* { enc : Enc.t *)
    (* ; pp : string option *)
    (* } *)
  ...
  module MakeFromEConstr (Ctx : Context.TYPE) : TYPE with type t = EConstr.t =
  struct
    type t = EConstr.t
...
module Constructor = struct
    ...
    module Make (T : Term.TYPE) : TYPE = struct
      module Node = struct
        ...
        let to_string ((lts, index) : t) : string =
          Utils.Strfy.record
            [ "lts", T.to_string lts; "index", Utils.Strfy.int index ]
        ;;
      end
  ...
  module Triple = struct
    ...
    module Make (T : Term.TYPE) = struct
      module Tree = Tree.Make (T)
...
module Monad = struct
  ...
  module Make (Ctx : Context.TYPE) (Enc : Encoding.TYPE) : TYPE = struct
    ...
    module Constructor : Constructor.TYPE =
      Constructor.Make (Term.MakeFromEnc (Enc))
...
module Model = struct
  module type TYPE = sig
    ...
    (* *)
    module CTree : Constructor.Tree.TYPE
    module CTrees : Set.S with type elt = CTree.t
  ...
  module Make (T : Term.TYPE) : TYPE = struct
    ...
    module State : SState = struct
      ...
      let equal (a : t) (b : t) : bool = T.equal a.term b.term
      let compare (a : t) (b : t) : int = T.compare a.term b.term
      let hash (x : t) : int = T.hash x.term

      (* *)
      let to_string (x : t) : string =
        Utils.Strfy.record
          [ "term", T.to_string x.term
    ...
    module Annotations : SAnnotations = struct
      ...
      let to_string (xs : t) : string =
        ...
        |> Utils.Strfy.list
             ~args:
               { (Utils.Strfy.style_args ()) with name = Some "Annotations" }
    ...
    module Transitions : STransitions = struct
      ...
      let to_string (xs : t) : string =
        ...
        |> Utils.Strfy.list
             ~args:
               { (Utils.Strfy.style_args ()) with name = Some "Transitions" }
