...
module MakeHTy (HTy : HTY_S) : HTY_TYPE = struct
  ...
  let opt_of_hty (sigma : Evd.evar_map) (p : Rocq_utils.kind_pair)
    : HTy.t option
    =
...
module MakeHyp (Hyp : HYP_S) : HYP_TYPE = struct
  ...
  let hty_is_a : Evd.evar_map -> Rocq_utils.kind_pair -> bool = Hyp.HTy.hty_is_a
...
module Make (HTy : HTY_S) : HYP_TYPE = MakeHyp (struct
    ...
    let of_hyp (sigma : Evd.evar_map) (h : Rocq_utils.hyp) : t =
      try HTy.of_hty sigma (Rocq_utils.hyp_to_atomic sigma h) with
