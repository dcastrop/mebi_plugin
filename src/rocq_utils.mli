...
exception
  Rocq_utils_EConstrIsNotA_Type of
    (Evd.evar_map * Evd.econstr * string)

val econstr_to_atomic :
  Evd.evar_map -> Evd.econstr -> Evd.econstr kind_pair
...
exception
  Rocq_utils_ConstrIsNot_App of (Constr.t * constr_kind)
...
exception
  Rocq_utils_EConstrIsNot_App of
    (Evd.evar_map * Evd.econstr * econstr_kind)
...
exception
  Rocq_utils_EConstrIsNot_Lambda of
    (Evd.evar_map * Evd.econstr * econstr_kind)

val econstr_to_lambda :
  Evd.evar_map -> Evd.econstr -> lambda_triple
...
exception
  Rocq_utils_HypIsNot_Atomic of
    (Evd.evar_map * hyp * EConstr.kind_of_type)

val hyp_to_atomic :
  Evd.evar_map -> hyp -> Evd.econstr kind_pair
...
val type_of_econstr_rel :
  ?substl:Evd.econstr list -> econstr_decl -> Evd.econstr
...
module Strfy : sig
  ...
  val ind_constr :
    Environ.env -> Evd.evar_map -> ind_constr -> string

  val ind_constrs :
    Environ.env -> Evd.evar_map -> ind_constr array -> string
  ...
  val name_id :
    ?args:Utils.Strfy.style_args -> Names.variable -> string

  val global :
    ?args:Utils.Strfy.style_args -> Names.GlobRef.t -> string
  ...
  val goal :
    ?args:Utils.Strfy.style_args -> Proofview.Goal.t -> string
end
