...
let check_equal
  ...
  let x : bool =
    if annotation
    then Option.equal Model_note.annotation_equal a.annotation b.annotation
    else true
  in
  let y : bool =
...
let check_compare
  ...
  let g = Option.compare Model_note.annotation_compare in
...
let to_string ?(args : style_args = record_args ()) (x : t) : string =
  ...
  let annotation : string =
    Option.cata (Model_note.annotation_to_string ~args) "None" x.annotation
  in
