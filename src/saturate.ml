...
module Make (M : Model.S) :
  S
  with module Model = M
   and module Model.Enc = M.Enc
   and type Model.Enc.t = M.Enc.t
   and module Model.Tree.Enc = M.Tree.Enc
   and type Model.Tree.Enc.t = M.Tree.Enc.t
   and type Model.Tree.TreeNode.t = M.Tree.TreeNode.t = struct
  module Model : Model.S with module Enc = M.Enc and type Enc.t = M.Enc.t = M
  ...
  module ActionPairs : SActionPairs with type elt = ActionPair.t = struct
