...
type json_action = (json_label_name * json_label_name) * json_action_silent
...
type json_edge =
  (json_edge_silent * json_edge_silent) * (json_edge_silent * json_edge_silent)

type model_info =
  { name : json_edge_silent
  ; kind : json_edge_silent
  ; extra : Model.Info.t option
  }
...
val label_to_str : Model.Action.Label.t -> string
val state_to_str : Model.State.t -> string
val state_opt_to_str : Model.State.t option -> string
val string_opt : string option -> string
val bool_opt : bool option -> string
...
val write_json_extra_to_file : out_channel -> Model.Info.t option -> unit
